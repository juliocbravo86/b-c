			...
			if($player != null){
				...
				foreach($playlist_arr as $i => $item){
					...
					foreach( $item->files as $file){
						...
						$item_src_obj->type = $config_obj->type.'/'.$ext;
					...
					if( isset($item->poster) ) $pl_item_obj->poster = $this->transform_url($item->poster);
					$pl_item_obj->source = $item_srcs;
					$pl_item_obj->track = $item_subtitles;
					
					$pl_items[] = '<li value="'.esc_attr( json_encode( $pl_item_obj ) ).'">'.((!empty($item->link)) ? '<a class="cpmp-info" href="'.esc_url($item->link).'">+</a>' : '&nbsp;&nbsp;').'&nbsp;&nbsp;'.$item->annotation.'</li>';
